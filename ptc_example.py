...
simdark = 5.0 # dark counts
simgain = 408/ng # effective gain factor
simsr = 43  # read noise

maxphotoelect = 20*ng # max signal in photoelectrons before gain
...
# Instead, make a continuous ramp and reshape which provides more intermediate pixels values
...
# model shot noise
if 1:  # apply shot noise to photoelectrons and then add dark and apply gain  
    for col in np.arange(simd.shape[2]):
        simd[:,:,col]=np.random.poisson(lam=simd[0,0,col],
                                        size=(frames,N))*simgain
    ...
    for col in np.arange(simd.shape[2]):
        simd[:,:,col]=np.random.poisson(lam=simd[0,0,col],
                                        size=(frames,N))
    simd *= (simgain+np.random.randn(frames,N,N)*30) # random gain
    simd += simdark # add dark counts
    simd += np.random.randn(frames,N,N)*simsr # add read noise using normal dist 
...
binnedsimdsignal, binnedsimdnoise = ptc.ptc(simd,binsize=1) # binned data
...
a2[0,1].imshow(simd.std(axis=0,ddof=1)); a1[0,1].set_title('modeled noise')
